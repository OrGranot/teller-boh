...
export default function RedeemClient({ code }: { code: string }) {
  ...
  async function handleRedeem() {
    ...
    if (res.ok) {
      setRedeemed(true);
      setVoucher((v) =>
        v ? { ...v, status: "redeemed", redeemed_at: new Date().toISOString() } : v
      );
    } else {
      ...
      <div style={styles.page}>
        <div style={styles.card}>
          ...
          <p style={{ color: "#6b7280", fontSize: 14 }}>
            Code: <code style={styles.mono}>{code}</code>
          </p>
        </div>
      </div>
    );
      ...
      <div style={styles.card}>
        ...
        {voucher.notes && (
          <div style={styles.notes}>
            <div style={{ fontSize: 11, color: "#9ca3af", letterSpacing: 1, marginBottom: 6, textTransform: "uppercase" }}>
              Notes
            </div>
        ...
        {isActive && !redeemed && (
          <div style={{ marginTop: 28, textAlign: "center" }}>
            {!confirming ? (
              <button onClick={() => setConfirming(true)} style={styles.redeemBtn}>
                Mark as Redeemed
              </button>
            ) : (
              <div>
                ...
                <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
                  ...
                  <button onClick={() => setConfirming(false)} style={styles.cancelBtn}>
                    Cancel
                  </button>
                </div>
              </div>
