  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
      <div className="flex items-center justify-between mb-5">
        ...
        <Link
          href="/invoices/new"
          className="bg-gray-900 text-white px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl text-sm font-semibold hover:bg-gray-700 transition-colors whitespace-nowrap"
        >
          + New invoice
        </Link>
      </div>
      ...
      <input
        ...
        className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-gray-800 bg-white mb-2"
      />

      {/* Status tabs */}
      <div className="flex items-center gap-1 bg-white border border-gray-200 rounded-xl px-2 mb-3 w-full sm:w-auto self-start">
        {(["all", "draft", "sent", "paid"] as const).map((s) => (
          <button
            key={s}
            onClick={() => setFilter(s)}
            className={`flex-1 sm:flex-none px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors capitalize ${
      ...
      {/* Date range filter — horizontal scroll on mobile */}
      <div className="overflow-x-auto -mx-4 px-4 sm:mx-0 sm:px-0 mb-4">
        <div className="flex gap-2 items-center min-w-max">
          ...
          <button onClick={() => setMonth(0)}
            className="text-xs px-3 py-2 rounded-xl border border-gray-200 bg-white text-gray-500 hover:text-gray-900 hover:border-gray-400 transition-colors whitespace-nowrap">
            This month
          </button>
            ...
            {filtered.map(inv => (
              <div key={inv.id} className="bg-white rounded-2xl px-4 py-3.5" style={{ boxShadow: "0 2px 12px rgba(0,0,0,0.05)" }}>
                <div className="flex items-start justify-between gap-2 mb-1">
                  ...
                  <span className="font-bold text-sm text-gray-900 whitespace-nowrap tabular-nums">
                    {formatEuro(inv.total)}
                  </span>
                </div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="ml-auto">
                    {inv.status === "paid" ? (
                      <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-green-50 text-green-700">✓ Paid</span>
                    ) : (
                      <select
                        value={inv.status}
                        onChange={e => updateStatus(inv.id, e.target.value as "draft" | "sent")}
                        className={`text-xs font-semibold px-2 py-0.5 rounded-full border-0 outline-none cursor-pointer ${STATUS_COLORS[inv.status]}`}
                      >
                ...
                <div className="flex gap-2 pt-3 border-t border-gray-100">
                  <button
                    ...
                    className="text-xs text-gray-500 font-semibold px-3 py-1.5 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors disabled:opacity-40 flex-1"
                  >
                    {downloading === inv.id ? "…" : "PDF"}
                  </button>
                  <Link
                    href={`/invoices/${inv.id}`}
                    className="text-xs text-gray-500 font-semibold px-3 py-1.5 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors flex-1 text-center"
                  >
                    Edit
                  </Link>
                  <button
                    onClick={() => setDeleteConfirm(inv.id)}
                    className="text-xs text-red-400 font-semibold px-3 py-1.5 rounded-lg bg-red-50 hover:bg-red-100 transition-colors flex-1"
                  >
                    Delete
                  </button>
                </div>
                ...
                {filtered.map((inv, i) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3.5 text-center whitespace-nowrap">
                      {inv.status === "paid" ? (
                        <span className="inline-block text-xs font-semibold px-2.5 py-1 rounded-full bg-green-50 text-green-700">✓ Paid</span>
                      ) : (
                        <select
                          value={inv.status}
                          onChange={e => updateStatus(inv.id, e.target.value as "draft" | "sent")}
