...
export default function VouchersClient({ restaurantId }: Props) {
  const supabase = createClient();
  const [vouchers, setVouchers] = useState<Voucher[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");
  const [filter, setFilter] = useState<
    "all" | "active" | "redeemed" | "expired" | "cancelled"
  >("all");
    ...
    <div className="max-w-5xl mx-auto px-6 py-10">
      ...
      <div className="flex gap-3 mb-4">
        ...
        <div className="flex items-center gap-1 bg-white border border-gray-200 rounded-xl px-2">
          {(
            ["all", "active", "redeemed", "expired", "cancelled"] as const
          ).map((s) => (
            <button
              key={s}
              onClick={() => setFilter(s)}
                      ...
                      <td colSpan={6} className="px-5 py-4">
                          ...
                          <div>
                            {v.personal_message && (
                              <div className="mb-3">
                                ...
                                <p className="mt-1 text-gray-700 italic">
                                  &ldquo;{v.personal_message}&rdquo;
                                </p>
                              </div>
                            )}
                            <div className="text-xs text-gray-400">
                            ...
                            {v.redeemed_at && (
                              <div className="text-xs text-gray-400 mt-1">
                                Redeemed on {formatDate(v.redeemed_at)}
                              </div>
                            )}
                            {v.stripe_session_id && (
                              <div className="text-xs text-gray-400 mt-1">
                                Stripe: {v.stripe_session_id}
                              </div>
                            )}
                          </div>
                          <div>
                        ...
                        <div className="mt-4 pt-4 border-t border-gray-200 flex items-center justify-between">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              downloadPdf(v.id, v.voucher_code);
                            }}
                          ...
                          {deleteConfirm?.id === v.id ? (
                            ...
                            >
                              <span className="text-xs text-red-600 font-semibold">
                                Type the voucher code to confirm deletion:
                              </span>
                              <input
                                type="text"
                                value={deleteInput}
                              ...
                              <button
                                ...
                                disabled={
                                  deleteInput !== v.voucher_code || deleting
                                }
