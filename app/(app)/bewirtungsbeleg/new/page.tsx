...
export default function NewBewirtungsbelegPage() {
  ...
  function selectCatalogItem(cat: CatalogItem, idx: number) {
    ...
    setSuggestions(null);
    document.querySelector<HTMLInputElement>(`[data-sum="${idx}"]`)?.focus();
  }
  ...
  function draftSub(draft: Draft): string {
    ...
    const savedTime = new Date(draft.savedAt).toLocaleTimeString("de-DE", { hour: "2-digit", minute: "2-digit" });
  ...
  return (
    <div className="px-6 py-10 flex gap-6 items-start">
      ...
      <div className="w-72 flex-shrink-0">
        <div className="bg-white rounded-2xl p-6 sticky top-8" style={{ boxShadow: "0 2px 20px rgba(0,0,0,0.07)" }}>
              ...
              {drafts.map(draft => (
                ...
                >
                  <div className="px-3 py-3 pr-8">
                    <p className="text-sm font-semibold text-gray-800 truncate">{draftLabel(draft)}</p>
