...
export default function CompanySettingsClient({ restaurantId }: Props) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if (logoFile) {
      ...
      if (error) {
        setLogoError(
          `Logo upload failed: ${error.message}. Make sure the "logos" bucket exists and is public in Supabase Storage.`
        );
        setSaving(false);
        return;
      }
  ...
  return (
    <PageShell>
      ...
      <p className="text-sm text-gray-400 mb-8">
        These details appear on every invoice.
      </p>
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <Section title="Legal Entity">
          ...
          <Field label="Trading / Display Name (optional — italic below legal name)">
            <Input
              ...
              onChange={(v) => setCompany({ ...company, display_name: v })}
        ...
        <Section title="Contact & Address">
          <Field label="Address" required>
            <Input
              ...
              placeholder="Pappelallee 29, 10437 Berlin"
              required
            />
          </Field>
          <Field label="Phone">
        ...
        <Section title="Banking & Tax">
          <Field label="IBAN">
            <Input
              value={company.iban || ""}
              onChange={(v) => setCompany({ ...company, iban: v })}
              placeholder="DE48…"
            />
          </Field>
          <Field label="BIC">
            <Input
              value={company.bic || ""}
              onChange={(v) => setCompany({ ...company, bic: v })}
              placeholder="DEUTDEBB101"
            />
          </Field>
          <Field label="VAT Number (USt-IdNr.)">
            <Input
              value={company.vat || ""}
              onChange={(v) => setCompany({ ...company, vat: v })}
              placeholder="DE449881127"
            />
          </Field>
          <Field label="Tax Number (St.-Nr. / Steuernummer)">
            <Input
              value={company.tax || ""}
              onChange={(v) => setCompany({ ...company, tax: v })}
              placeholder="37/250/51293"
            />
          </Field>
          <Field label="Trade Register (Handelsregister)">
            <Input
              value={company.trade_register || ""}
              onChange={(v) => setCompany({ ...company, trade_register: v })}
              placeholder="HRB 267127 B"
            />
          </Field>
        </Section>
        ...
        <Section title="Invoice Counter">
          <p className="text-xs text-gray-400 mb-3">
            ...
            <strong className="text-gray-700">#{nextNum}</strong>. Enter a
