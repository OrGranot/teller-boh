...
// Server-to-server webhook — authenticated via Bearer secret.
// The caller must pass restaurant_id in the request body.
  ...
  try {
    ...
    if (!customer_name || !customer_email || !items?.length) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: customer_name, customer_email, items",
        },
        { status: 400 }
      );
    }
    ...
    const invoiceNumber = await getNextInvoiceNumber(supabase, restaurant_id);
    ...
    const subtotal = parsedItems.reduce(
      (
        acc: number,
        item: { qty: string; price: string; vat_rate: string }
      ) => {
    ...
    // Save / update contact (scoped to restaurant)
